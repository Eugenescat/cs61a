...
class Coin:
    ...
    def worth(self):
        extra = max(Mint.present_year - self.year - 50, 0)
...
def store_digits(n):
    ...
    if get_len(n) == 1:
        ...
        cur = n // (10** (get_len(n) - 1))
        n = n % (10** (get_len(n) - 1))
        return Link(cur, store_digits(n))
...
def two_list(vals, amounts):
    ...
    for i in range(len(vals)):
        merge = merge + [vals[i]] * amounts[i]

    def construct(merge):
        if len(merge) > 1:
            return Link(merge[0], construct(merge[1:]))
        else:
            return Link(merge[0])

    return construct(merge)
