...
def roman_numerals(text):
    ...
    []
    """
    # matches one or more of the Roman numeral characters
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'(?i)CS\s?\d+[ABC]?', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    # (?<!\.) ensures that the match is not preceded by a dot
    return re.findall(r'(?<!\.)\b\d{1,2}:\d{2}\b|(?<!\.)\b\d{1,2}:\d{2}[APap][Mm]\b', text)
...
def most_common_code(text):
    ...
    for i in range(len(codes)):
        if codes[i] in cache:
            continue
        cache.append(codes[i])
        n = codes.count(codes[i])
        if max < n:
            max = n
            code = codes[i]
    return code
