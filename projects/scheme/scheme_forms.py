...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        proc = do_lambda_form(Pair(formals, body), env)
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 5
    # in the original pair, pair.first is 'quote', pair.rest is expressions
    # pair.rest looks like: (something to be quoted)[first] nil[rest]
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if clause.rest is nil:
                # attention: besides #t, test can be any true value, e.g. test = 12
                return test
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        name = binding.first
        # attention: binding.rest.first, not binding.rest
        # e.g. (x (+ 1 2))  Pair('x', Pair(Pair('+', Pair(1, Pair(2, nil))), nil))
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 11
    body = expressions.rest
    mu_proc = MuProcedure(formals, body)
    return mu_proc
    # END PROBLEM 11
